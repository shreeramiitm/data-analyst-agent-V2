...
async def analyze_data(
    ...
    try:  # Main API endpoint for data analysis
        ...
        # Execute workflow synchronously (always within 5 minutes)
        logger.info(
        ...
        try:
            ...
            result = await asyncio.wait_for(
                # Change the hardcoded timeout to use the constant
                execute_workflow_sync(detected_workflow, workflow_input, task_id), timeout=TIMEOUT # <--- CHANGED THIS
            )
            ...
            # This is the crucial change to return a structured JSON on timeout
            logger.error(f"Task {task_id} timed out after 5 minutes")
            # Instead of raising an HTTPException, return a JSON response
            return {
                "task_id": task_id,
                ...
                "result": {
                    ...
                    "message": "Returning a structured response as required.",
                    "answers": [] # Return an empty array for the answers
                },
                "processing_info": {
                    "questions_file": questions_txt.filename,
                    "additional_files": list(processed_files.keys()),
                    "workflow_auto_detected": True,
                    "processing_time": "exceeded_5_minutes",
                },
                "timestamp": datetime.now().isoformat(),
            }
        except Exception as e:
